...
def hellaswag_acc(model, enc, device, max_examples: int = 1000) -> float:
    ...
    For each (ctx, ending), we score `log P(ending | ctx)` by feeding the full
    sequence through the model once with `return_full_logits=True` and
    averaging per-token log-probs over the completion's positions.
    """
    ...
    for ex in ds.select(range(min(max_examples, len(ds)))):
        ...
        for ending in ex["endings"]:
            ...
            inp = ids[:, :-1]; tgt = ids[:, 1:]
            # Single forward with full per-position logits.
...
def main():
    ...
    if "ppl" in tasks:
        ...
        ds = ShardDataset(os.path.join("data", cfg["dataset"]),
                          cfg["model"]["block_size"], device, split="val")
