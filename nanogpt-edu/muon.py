...
Scope rules (important):
  * Muon is for 2D *hidden* weight matrices only (attn q/k/v/o, MLP weights).
  * Embeddings, the lm_head, all 1-D params (RMSNorm gains, biases), and any
    scalar should be optimized by a standard method (AdamW) instead.
...
def split_muon_params(model):
    ...
    Muon takes 2D hidden weight matrices. Embeddings and the lm_head are 2D too
    but are excluded by name — they're the input/output layers, which Muon's
    author recommends keeping on AdamW. Everything 1-D (norms, biases) → AdamW.
    """
